...
async function customCloneKey(keyToClone: any, retryCount = 0) {
  ...
  try {
    ...
    keyDispatcher.addKey(key.account.license);
...
async function worker(id: number) {
  ...
  if (key !== null) {
    ...
    const output = config.OUTPUT_FORMAT.replace(/\\n/g, '\n')
      ...
      .replace(/{private_key}/g, key.private_key)
      .replace(/{id}/g, key.id)
      ...
      .replace(
        /{account.referral_renewal_countdown}/g,
        key.account.referral_renewal_countdown
      )
